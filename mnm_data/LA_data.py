...
for i in range(160):
    ...
    to_format_img.to_filename(os.path.join(path,name+'_LA_ES'+'.nii.gz'))
    ...
    to_format_gt.to_filename(os.path.join(path,name+'_LA_ES_gt'+'.nii.gz'))
    ...
    to_format_img = nib.Nifti1Image(img, np.eye(4))  
    to_format_img.set_data_dtype(np.uint8)
    to_format_img.to_filename(os.path.join(path,name+'_LA_ED'+'.nii.gz'))
        
        ###  saving the gts 
    gt = np.moveaxis(gt, [0, 1, 2], [-1, -2, -3])   ## reverse the dimenssion sof array
    to_format_gt = nib.Nifti1Image(gt, np.eye(4))  
    to_format_gt.set_data_dtype(np.uint8)
    to_format_gt.to_filename(os.path.join(path,name+'_LA_ED_gt'+'.nii.gz'))
